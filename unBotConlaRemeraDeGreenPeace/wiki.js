import fs from "fs";
import { Network, Architect, Neuron } from "synaptic";

const dictionary = "0123456789qwertyuiopasdfghjklzxcvbnm,.()'- ".split("");
const keys = dictionary.reduce((acc, letter, i) => {
  acc[letter] = +i;
  return acc;
}, {});
let charsRead = 0;
...
const getNetwork = () => {
  try {
    ...
    return new Architect.LSTM(dictionary.length, 60, dictionary.length);
  }
};
...
function init() {
  ...
  Wiki.articles = localStorage.getItem("articles")
    ? localStorage.getItem("articles").split("&")
    : [Wiki.article];
  Wiki.characters = localStorage.getItem("characters")
    ? +localStorage.getItem("characters")
    : 0;
  Wiki.currentParagraph = localStorage.getItem("currentParagraph")
    ? +localStorage.getItem("currentParagraph")
    : 0;
  Wiki.currentWord = localStorage.getItem("currentWord")
    ? +localStorage.getItem("currentWord")
    : 0;
  Wiki.time = localStorage.getItem("time") ? +localStorage.getItem("time") : 0;
  Wiki.learningRate = localStorage.getItem("learningRate")
  ...
  const quantity = Neuron.quantity();
      ...
      if (Wiki._stop) {
        ...
        requestAnimationFrame(function() {
          ...
          localStorage.setItem(
            "network",
            JSON.stringify(network || Net.toJSON())
          );
  ...
  Wiki.play = function() {
    ...
    $("#export-button").css("opacity", 0.5);
    $("#load-button").css("opacity", 0.5);
    log(
      "Reading paragraph " + (Wiki.currentParagraph + 1) + "/" + Wiki.p.length,
      "status"
    );
    readParagraph();
  };
...
function loadArticle(article, startFromBegining) {
  ...
  for (var i in Wiki.articles)
    if (article == Wiki.articles[i]) alreadyRead = true;
  if (!alreadyRead) Wiki.articles.push(article);
  log(article, "article");
...
function readParagraph(nth) {
  if (typeof nth == "undefined") nth = Wiki.currentParagraph;
  else Wiki.currentParagraph = nth;
...
function readWord(nth) {
  ...
  if (charsRead > maxChars) return Wiki.save();

  if (typeof nth == "undefined") nth = Wiki.currentWord;
  else Wiki.currentWord = nth;
  ...
  var input = [];
  var target = [];
  for (var i = 0; i < length; i++) input[i] = 0;
  input[keys[" "]] = 1;
  ...
  const sendMessage = function(action) {
    if (chars[index] in keys) {
      for (var j = 0; j < length; j++) target[j] = 0;
  ...
  Wiki.worker.onmessage = function(e) {
    ...
    if (e.data.action === "activate") {
      ...
      for (var j = 0; j < length; j++) input[j] = 0;
      ...
      if (targetChar === predictedChar) success++;
      ...
      if (targetChar === predictedChar) next();
      else sendMessage("propagate");
    } else {
      next();
    }
  };
